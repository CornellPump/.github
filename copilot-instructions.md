# Org-Wide Copilot Review Instructions

Review scope: syntax errors, style violations, and null/None safety only.
Do NOT comment on logic, architecture, or anything outside these three categories.
...
- Null safety: check for null/undefined before accessing properties on values
  that could be either (API responses, DOM queries, optional chaining preferred)

## JavaScript (.js)
- 2-space indentation
- Prefer const > let, never var
- Null safety: guard values from fetch responses, localStorage, Redux store and querySelector
  before use; optional chaining (?.) encouraged
